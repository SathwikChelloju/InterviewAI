...
public class EmailServiceImpl implements EmailService {
    ...
    public void sendVerificationCode(String email, String code) {
        try {
            // MimeMessage is required to send HTML formatting
            MimeMessage message = mailSender.createMimeMessage();
            
            // The 'true' parameter allows multipart messages (HTML content)
            MimeMessageHelper helper = new MimeMessageHelper(message, true, "UTF-8");

            helper.setTo(email);
            helper.setSubject("InterviewMate Email Verification");

            // The exact HTML template. We use {{OTP_CODE}} as a placeholder.
            String htmlContent = """
                    <!DOCTYPE html>
                    <html lang="en">
                    <head>
                    ...
                    """.replace("{{OTP_CODE}}", code); // Replaces the placeholder with the actual code

            // The 'true' flag here tells Spring Boot to send the email as HTML
